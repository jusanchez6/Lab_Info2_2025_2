...
{
    string str_cost;
    cout << "Ingrese los routers a enlazar" << endl;
...
void consult_shortest_route(Network &net)
{
    ...
    cout << "Ingrese los routers de inicio y finalizacion." << std::endl;
...
void consult_package_cost(Network &net)
{
    ...
    if (cost < min_cost)
        return;
    cout << "Enviar un paquete desde " << router_id_1 << " a " 
              << router_id_2 << " cuesta: " << cost << endl; 
}
...
bool get_router_id(string &router_id)
{
    cout << "ID del router: ";
    cin >> router_id;
